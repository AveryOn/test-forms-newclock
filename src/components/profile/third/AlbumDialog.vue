    ...
    <v-card-text>
      <v-form v-model="isFormValid" ref="refAlbumForm" lazy-validation>
        ...
        <v-checkbox
          ...
          class="pa-0"
          hide-details="false"
          v-model="albumData.is_private"
        ></v-checkbox>
        <v-card-subtitle class="pa-0 pl-7"
          >Фото из этого альбома будете видеть только Вы</v-card-subtitle
        >
      </v-form>
    </v-card-text>
    <v-card-text class="d-flex justify-end pb-5">
      <v-btn variant="text">Отмена</v-btn>
      <v-btn color="primary" class="ml-4" @click="addedetAlbum"
        >Создать альбом</v-btn
      >
    </v-card-text>
  </v-card>
</template>
...
const addedetAlbum = async () => {
  const { valid } = await refAlbumForm.value.validate();
  if (valid) {
    createAlbum(albumData.value).then(() => {
      emit("update");
    });
  }
};
