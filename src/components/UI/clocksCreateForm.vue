<template>
    <div v-if="!modelValue" class="input-container d-flex align-center no-select mt-2 img-avatar" @dragleave="onDragLeave"
        ...
        style="border-radius: 15px; width: 168px; height: 168px" :style="{}">
        <label :for="`${uid}` + '-input'"
            ...
            <slot name="empty-layout d-flex align-center">
                <v-icon icon="mdi-play-outline" class="text-h4" color="primary"></v-icon>
            </slot>
        </label>
    </div>
    <div :style="{}" @dragleave="onDragLeave" @dragover.prevent="onDragOver"
        style="width: 168px; height: 168px; border-radius: 15px" v-else class="relative no-select selected-image mt-2">
